...
export async function syncSupabaseUserToStudent(email: string) {
  try {
    ...
    console.error('Error in syncSupabaseUserToStudent server action:', err)
    return { success: false, error: err.message || 'Failed to sync Supabase user. Please check server logs.' }
  }
}
