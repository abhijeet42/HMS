...
export async function createStudentUser(email: string, fullName: string) {
  try {
    ...
    console.error('Error in createStudentUser server action:', err)
    return { 
      success: false, 
      error: err.message || 'Failed to initialize database admin client. Please verify that SUPABASE_SERVICE_ROLE_KEY is set in your environment variables.'
    }
  }
}
...
export async function deleteStudentUser(studentId: string, userId: string | null) {
  try {
    ...
    console.error('Error in deleteStudentUser server action:', err)
    return { success: false, error: err.message || 'Failed to delete student user.' }
  }
}
...
export async function resetStudentPassword(userId: string, newPassword: string) {
  try {
    ...
    console.error('Error in resetStudentPassword server action:', err)
